...
- **Configurable Uvicorn Workers**: Added `workers` configuration for local server mode. Configure via `[plugins.scale.server]` in `jac.toml` (`workers = 4` or `workers = "auto"` for CPU-based scaling) or override via CLI (`jac start app.jac --workers 4` / `-w auto`). Default is 1 worker. Note: Workers apply to local mode only; Kubernetes deployments use pod replicas for scaling.
